...
public class JigsawNode {
  private static int dimension; //拼图的维度
   ...
   *             按照先行后列排序）。
   */
  public JigsawNode(int[] data,int command) {
	  JigsawNode.dimension = command;
    if (data.length == JigsawNode.dimension * dimension + 1) {
   ...
   * @param jNode - 用以复制的节点
   */
  public JigsawNode(JigsawNode jNode) {//构造函数
